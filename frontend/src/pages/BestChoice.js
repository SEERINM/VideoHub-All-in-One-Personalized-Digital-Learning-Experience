  ...
  return (
    ...
    <div style={styles.page}>
      <div style={styles.card}>
        <h2 style={styles.title}>🎯 Best Choice Video Analyzer</h2>
        <form onSubmit={handleSubmit} style={styles.form}>
       
          <label style={styles.label}>Enter Topic:</label>
          <input
            type="text"
            value={topic}
            onChange={(e) => setTopic(e.target.value)}
          ...
          <label style={styles.label}>Select Number of Videos:</label>
          <select
            value={numVideos}
            onChange={(e) => {
              setNumVideos(parseInt(e.target.value));
              setVideos([]);
            }}
            style={styles.select}
            disabled={loading}
          ...
          <div style={styles.uploadSection}>
            {Array.from({ length: numVideos }).map((_, i) => (
              <div key={i} style={styles.uploadBlock}>
                <label style={styles.uploadLabel}>Upload Video {i + 1}:</label>
                <input
                  type="file"
                  accept="video/*"
                  onChange={(e) => handleFileChange(i, e.target.files[0])}
          ...
          <button type="submit" style={styles.button} disabled={loading}>
            {loading ? "Analyzing..." : "Choose the Best"}
          </button>
        </form>
        ...
        {bestVideo && (
          <div style={styles.result}>
            <h3 style={styles.resultTitle}>🏆 Best Video:</h3>
            <p style={styles.resultText}>{bestVideo}</p>
          </div>
        )}
      </div>
    </div>
  ...
  return (
    <div className="form-container">
      <form onSubmit={handleSubmit}>
        ...
        <label>Select number of videos:</label>
        <select
          value={numVideos}
          onChange={(e) => {
            setNumVideos(parseInt(e.target.value));
