...
export default function AnswerBot() {
  ...
  return (
     <>
        <Navbar/>
    <div className="answerbot-wrapper">
      <div className="answerbot-card">
        <div className="chat-header">🎥 AnswerBot</div>

        {!ready && !ended && (
          <div className="setup">
            <h3>Upload your videos to begin</h3>
            <p>Select how many videos you want to analyze:</p>
          ...
          <div className="chat-box">
            <div className="messages">
              {chat.map((msg, i) => (
                <div
                  key={i}
                  className={`msg ${msg.sender === "You" ? "user" : "bot"}`}
                >
            ...
            <div className="input-row">
              ...
              <button onClick={handleEndChat} className="stop-btn">
                End Chat
              </button>
            </div>
          </div>
        ...
        {ended && (
          <div className="end-box">
            <p>🧠 Chat session ended. Want to start again?</p>
...
export default function AnswerBot() {
  ...
  // ✅ Automatically include login token from localStorage
  const authHeaders = () => {
    const token = localStorage.getItem("token");
    return token ? { Authorization: `Bearer ${token}` } : {};
  };
  ...
  // ------------------------------
  // Ask question → backend answers using RAG
  // ------------------------------
  const handleAsk = async () => {
    if (!question.trim()) return;
  ...
  // ------------------------------
  return (
    <div className="answerbot-container">
      <div className="chat-header">🎥 AnswerBot</div>
