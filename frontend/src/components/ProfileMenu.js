  ...
  return (
    <div style={{position:"relative"}}>
      <button className="profile-btn" onClick={()=>setOpen(s=>!s)}>
        <span style={{width:32,height:32,borderRadius:16,background:"#fff3",display:"inline-block",textAlign:"center",lineHeight:"32px"}}>👤</span>
      </button>
        ...
        <div className="profile-menu" onMouseLeave={()=>setOpen(false)}>
          {user ? (
            <>
              ...
              <div style={{height:10}} />
              <button onClick={logout} style={{padding:"8px 12px",borderRadius:6,border:"none",cursor:"pointer"}}>Logout</button>
            </>
          ) : (
            <>
              <p>Guest</p>
              <p className="small">Not logged in</p>
              <div style={{height:10}} />
              <button onClick={()=>nav("/login")} style={{padding:"8px 12px",borderRadius:6}}>Login</button>
            </>
          )}
        </div>
      )}
